...
class ScrapegraphaiTest < Minitest::Test
  ...
  def test_raises_on_unknown_environment
    e = assert_raises(ArgumentError) do
      Scrapegraphai::Client.new(environment: "wrong")
    end
  ...
  def test_raises_on_missing_non_nullable_opts
    e = assert_raises(ArgumentError) do
      Scrapegraphai::Client.new
    end
    assert_match(/is required/, e.message)
  end

  def test_client_default_request_default_retry_attempts
    stub_request(:post, "http://localhost/smartscraper").to_return_json(status: 500, body: {})

    scrapegraphai = Scrapegraphai::Client.new(base_url: "http://localhost", api_key: "My API Key")

    assert_raises(Scrapegraphai::Errors::InternalServerError) do
  ...
  def test_client_given_request_default_retry_attempts
    ...
    scrapegraphai =
      Scrapegraphai::Client.new(base_url: "http://localhost", api_key: "My API Key", max_retries: 3)
  ...
  def test_client_retry_after_seconds
    stub_request(:post, "http://localhost/smartscraper").to_return_json(
      status: 500,
    ...
    scrapegraphai =
      Scrapegraphai::Client.new(base_url: "http://localhost", api_key: "My API Key", max_retries: 1)
  ...
  def test_client_redirect_307
    ...
    assert_raises(Scrapegraphai::Errors::APIConnectionError) do
      scrapegraphai.smartscraper.create(
        user_prompt: "Extract the product name, price, and description",
    ...
    assert_requested(:any, "http://localhost/redirected", times: Scrapegraphai::Client::MAX_REDIRECTS) do
  ...
  def test_client_redirect_303
    ...
    assert_requested(:get, "http://localhost/redirected", times: Scrapegraphai::Client::MAX_REDIRECTS) do
  ...
  def test_client_redirect_auth_strip_cross_origin
    ...
    assert_requested(:any, "https://example.com/redirected", times: Scrapegraphai::Client::MAX_REDIRECTS) do
      headers = _1.headers.keys.map(&:downcase)
  ...
  def test_default_headers
    stub_request(:post, "http://localhost/smartscraper").to_return_json(status: 200, body: {})
